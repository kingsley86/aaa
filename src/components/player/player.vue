...
export default {
  ...
  methods: {
    ...
    handleLyric(obj) {
      ...
      if (obj.lineNum > 5) {
        let lineEl = this.$refs.lyricLine[obj.lineNum - 5]
        this.$refs.lyricList.scrollToElement(lineEl, 1000)
      } else {
        this.$refs.lyricList.scrollTo(0, 0, 1000)
      }
      this.playingLyric = obj.txt
    },
    showPlaylist() {
    ...
    middleTouchStart(e) {
      ...
      /* 是否为一次移动 */
      /* 如果不进行move的判断的话，第一次程序是正常的的，但是由于第一次记住了percent，于是第二次开始 */
      /* end直接对它进行操作，所以就会出现这种点击就跳转的操作，因此需要设置辅助变量看他是否有经过move过程 */
